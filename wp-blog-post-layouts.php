...
define( 'WPBLOG_POST_LAYOUTS_VERSION', '1.1.4' );
...
if ( !function_exists( 'wpblog_post_layouts_admin_notice' ) ):
	...
	?>
			<div id="cv-blog-post-layouts-message" class="blog-post-layouts-message notice notice-info is-dismissible">
				<p><?php
					/* translators: %1$s is plugin name %2$s and %3$s for href link of translations */
					printf( esc_html__( 'Thank you for choosing %1$s! To fully take advantage of the best our plugin can offer please make sure you visit our %2$s dashboard page %3$s.', 'wp-blog-post-layouts' ), esc_html( WPBLOG_POST_LAYOUTS ), '<a href="'.esc_url( admin_url( 'admin.php?page=wp-blog-post-layouts' ) ).'">', '</a>' ); ?></p>
...
if ( !function_exists( 'wpblog_post_layouts_gutenberg_admin_notice' ) ) :
	...
	function wpblog_post_layouts_gutenberg_admin_notice() {
		...
		$message = sprintf(
			/* translators: %1$s for gutenberg not installed and %2$s gutenberg blocks of translations */
